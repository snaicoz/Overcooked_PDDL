...
def parse_pddl_output(stdout_text):
    ...
    for line in stdout_text.splitlines():
        ...
        if match:
            ...
            action_name = parts[0]
            args = parts[1:]
            agent = next((a for a in args if a.startswith('c')), "c1")
            if agent not in agents_plan:
                agents_plan[agent] = []
...
def show_timestamp():
    ...
    timestamp_pen.write(f"TIMESTAMP: {current_time}", align="center", font=("Courier New", 21, "bold"))

def play(x=0, y=0):
    global is_paused
    if agents_plan and is_paused:
        is_paused = False
...
def stop(x=0, y=0):
    ...
    # Genera una finestra modale con pulsanti Sì/No
    change_level = tkinter.messagebox.askyesno(
        "Pausa", 
        "Vuoi cambiare livello?\n\nScegli 'Sì' per caricare un nuovo livello, oppure 'No' per riprendere questo."
    )
    
    if change_level:
        # Riutilizziamo la tua funzione esistente che invoca il caricamento di un nuovo piano
        ask_next_level()
...
def restart(x=0, y=0):
    ...
    for obj_id in graphic_objects:
        if "p" in obj_id:
            ...
            graphic_objects[obj_id].shape("circle")
            graphic_objects[obj_id].color("grey")
...
def animation_cycle():
    ...
    if time_index >= len(time_list):
        is_paused = True
        btn_resume.shape("play_normale.gif")    # Il pulsante play torna al suo stato non premuto
        print("Simulazione completata.", flush=True)
        screen.ontimer(ask_next_level, 800)     # Chiede quale livello eseguire successivamente dopo 800 ms
        return
    ...
    # Aggiorna taccuino
    log_lines = []
    for c_id in sorted(agents_plan.keys()):
    ...
    for c_id, actions in agents_plan.items():
        current = [a for a in actions if a["time"] == current_time]     # Estratta azione corrente
        for act in current:
            ...
            if "moveto" in name:
                ...
                if destination in interaction_map and cook_id in cooks:
                    ...
                    steps = 100     #questo parametro influenza la velocità dello spostamento
            ...
            # Se l'azione è add_ingredient_to_plate:
                ...
                # Assegnazione immagine corretta
                shape = PLATE_SHAPE_MAP.get(frozenset(ps_comp[plate_slot]))
                if shape:
    ...
    time_index += 1
    screen.ontimer(animation_cycle, 600)    # Stabilisce la durata di ogni step
...
def load_level(chosen_level):
    ...
    try:
        ...
        with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
            f.write(f"--- PIANO GENERATO PER IL level {int(chosen_level)} ---\n\n")
...
# Fa comparire la finestra di dialogo per scegliere il livello da eseguire
def ask_next_level():
...
# Scritta stampata a schermo per visualizzare il timestamp
...
# Scritta di "Caricamento..." e "Premi play per iniiziare..."
...
# Scritte sul taccuino delle azioni eseguite dai cuochi per ogni timestamp
...
# Scritta per tracciamento ordini totali e già completati
...
# --- CREAZIONE LINEA NERA DI SEPARAZIONE (OGGETTO) ---
separator = turtle.Turtle()
separator.penup()                
separator.shape("square")        
separator.color("black")         

# Modifico dimensioni del quadrato per farlo diventare una linea verticale
separator.shapesize(stretch_wid=44, stretch_len=0.70)

# Posizionamento al confine esatto tra cucina e notebook (x = 650)
separator.goto(770, 0)
separator.showturtle()

screen.update()     # Aggiorna lo schermo in modo sincronizzato
screen.tracer(1)    # Sbloca di nuovo le animazioni non sincronizzate

# Avvio iniziale
chosen_level = screen.numinput("Overcooked", "Scegli il level (1-5):", minval=1, maxval=5)
if chosen_level:
